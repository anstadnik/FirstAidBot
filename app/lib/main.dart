...
class _FARHomePageState extends State<FARHomePage> {
  late Future<MultilangFs> faMLFS;
  RwLockFaContext faCTX = api.getContext();
  late Future<FAState?> faState;
  ...
  Widget build(BuildContext context) {
    return PlatformScaffold(
      appBar: PlatformAppBar(
        title: const Text('First Aid Robot'),
      ),
      body: Center(
        child: Column(
          ...
          children: [
            ...
            PlatformTextButton(
              onPressed: () {
                setState(() {
                  faMLFS = api.getData();
                  faState =
                      faMLFS.then((mlfs) => api.getFs(mlfs: mlfs, ctx: faCTX));
            ...
            FutureBuilder<List<dynamic>>(
              future: Future.wait([faMLFS, faState]),
              builder: (context, snap) {
                ...
                return Column(
                  children: [
                    Text(faState_.message),
                    ...faState_.nextStates.map((e) => PlatformElevatedButton(
                          onPressed: () {
                            setState(() {
                              api.transition(ctx: faCTX, text: e);
                              faState = faMLFS.then(
                                  (mlfs) => api.getFs(mlfs: mlfs, ctx: faCTX));
                            });
                          },
                          child: Text(e),
                        ))
